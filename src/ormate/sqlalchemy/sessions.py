...
class SessionScope:
    def __init__(self, db: Any, session: Session | None = None, *, reuse_current: bool = True) -> None:
        self.db = db
        self.bound_session = session
        self.reuse_current = reuse_current
        self.token: Token[Any] | None = None
        self.owner_token: Token[Any] | None = None
    ...
    def __enter__(self) -> Session:
        ...
        elif self.reuse_current and current is not None and self.db._session_is_current():
            self.session = current
        else:
            self.session = self.db.session_maker()
            self.owned = True
        self.token = self.db._session_context.set(self.session)
        self.owner_token = self.db._session_owner_context.set(self.db._context_owner())
        return self.session
    ...
    def __exit__(
        ...
        try:
            ...
            if self.owner_token is not None:
                self.db._session_owner_context.reset(self.owner_token)
            if self.token is not None:
                self.db._session_context.reset(self.token)


class AsyncSessionScope:
    def __init__(self, db: Any, session: AsyncSession | None = None, *, reuse_current: bool = True) -> None:
        self.db = db
        self.bound_session = session
        self.reuse_current = reuse_current
        self.token: Token[Any] | None = None
